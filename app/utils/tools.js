...
const MAX_MSG_LEN = 1024 * 8; // 允许对端发送的最大包长(字节单位)
const MAX_MSG_CNT = 4096;      // 允许缓冲区里最多容纳打消息个数
...
tools.parseMessageData = function(buffer, dataLen, maxMsgLen, maxMsgCnt){
  ...
  let idx = 0;
  while(true) {
    let msgLen = buffer.readUInt32LE(offset);
    if (msgLen === dataLen){
